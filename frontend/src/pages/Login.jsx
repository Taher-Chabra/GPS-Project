...
function Login() {
  ...
  async function loadCaptcha() {
    try {
      ...
      captchaRef.current.innerHTML = response.data;
    } catch (error) {
  ...
  return (
    <section className="w-full h-full max-w-md my-2 p-4 bg-white border border-gray-100 rounded-lg shadow-md shadow-gray-700 dark:shadow-gray-500 sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col p-4">
        ...
        <div className="mb-6">
          <div className="flex flex-col xs:flex-row items-center justify-between gap-5 ">
            <input
              type="text"
              id="_captcha"
            ...
            />
            <div>
              <span className="inline-flex items-center" ref={captchaRef}>
              </span>
            </div>
          </div>
          {errors.captchaToken && (
